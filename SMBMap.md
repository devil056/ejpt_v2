...
```
smbmap -u <username> -p <password> -d <directory-to-access> -H <target-ip>
```

#### Running a command

```
smbmap -u <username> -p <password> -d <directory-to-access> -H <target-ip> -x <ipconfig>
```

#### List the drives

```
smbmap -u <username> -p <password> -d <directory-to-access> -H <target-ip> -L
```

#### Listing the files

```
smbmap -u <username> -p <password> -H <target-ip> -r <drive-to-list>
```

There is a change in the target-ip as the lab session was reset in the middle. 10.5.18.55

#### Uploading file

```
smbmap -u <username> -p <password> -H <target-ip> --upload /file/path /dest/path
```

#### Downloading file

```
smbmap -u <username> -p <password> -H <target-ip> --download <file/path>
```

#### Deleting file

```
smbmap -u <username> -p <password> -H <target-ip> --delete <file/path>
```
